    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                ...
                if (!token) {
                    return [2 /*return*/, res.status(401).json({ message: "未授权，请先登录" })];
                }
                decoded = jsonwebtoken_1.default.verify(token, config_1.config.jwtSecret);
                return [4 /*yield*/, User_1.User.findById(decoded.userId).select("-password")];
            case 1:
                user = _b.sent();
                if (!user) {
                    return [2 /*return*/, res.status(401).json({ message: "用户不存在" })];
                }
                req.user = user;
                next();
                return [3 /*break*/, 3];
